...
class TurtleBot:
    ...
    def move2goal(self):
        ...
        self.target=int(input("Enter the target angle:"))
        ...
        vel_msg.angular.z = 0
        self.velocity_publisher.publish(vel_msg)
        self.target=self.target+self.theta
        while not rospy.is_shutdown():
            
            target_rad = self.target*pi/180
            vel_msg.angular.z = self.kp*(target_rad-self.yaw)
            self.velocity_publisher.publish(vel_msg)
            print(self.target)
            print(self.theta)
            self.rate.sleep()
